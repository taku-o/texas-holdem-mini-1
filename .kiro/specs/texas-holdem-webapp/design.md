...
| Styling | CSS / CSS Modules または Tailwind のいずれか | Apple風レイアウト・余白・タイポグラフィ | 実装時に選定 |
| Runtime | ブラウザ | 実行環境 | サーバー・DB なし |
| 役判定 | 既存 TypeScript/JS ライブラリ 1 つ | 7枚→役ランク | 実装時に選定、アダプタでラップ |
...
- **ゲーム終了条件（要件 8.3）**: 次のいずれかに該当したら phase を Idle に遷移し、必要に応じて「新しいゲームを始める」を表示する。(1) 人間プレイヤーのチップが 0 になった場合。(2) ユーザーが終了を選択した場合。
